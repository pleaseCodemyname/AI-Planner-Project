...
app.put("/goal/update/:event_id", requireLogin, upload.single("image"), async (req, res) => {
  const event_id = req.params.event_id; //evet_id를 통해 값을 받아옴
  const { title, startDatetime, endDatetime, location, content } = req.body; //Client에서 입력해야할 값
  ...
  try {
    ...
    if (imageUrl) {
      updateFields.push('#PhotoURL = :imageUrl');
    }
...
app.put("/event/update/:event_id", requireLogin, async (req, res) => {
  ...
  // 업데이트할 필드와 값을 포함한 업데이트 데이터
  const updateData = marshall({
    ':title': title,
    ':startDatetime': startDatetime,
    ':endDatetime': endDatetime,
    ':goal': goal,

...
app.get("/goal/read/:event_id", requireLogin, async (req, res) => {
  ...
  try {
    ...
    const goal = {
      ...
      isCompleted: response.Item.isCompleted ? true : false,
    };
...
app.put("/goal/update/:event_id", requireLogin, upload.single("image"), async (req, res) => {
  ...
  const { title, startDatetime, endDatetime, location, content, isCompleted } = req.body; // isCompleted 값을 요청에서 추출

  try {
    let imageUrl = null;

    if (req.file) { // 새로운 이미지가 업로드되었는지 확인
    ...
    const updateParams = {
      ...
      ExpressionAttributeValues: {
        ...
        ':location': { S: location },
        ':content': { S: content },
        ':isCompleted': { BOOL: isCompleted }, // isCompleted 값을 DynamoDB BOOL 형식으로 설정
      }
    };
...
app.get("/todo/complete/:event_id", requireLogin, async (req, res) => {
  ...
  try {
    ...
    const getParams = {
      ...
      ProjectionExpression: 'IsCompleted' // "IsCompleted" 필드로 수정
    };

    const getCommand = new GetItemCommand(getParams); // 'newGetItemCommand'에서 'new GetItemCommand'로 수정
    const getResponse = await dynamodbClient.send(getCommand); // 'sent'에서 'send'로 수정
